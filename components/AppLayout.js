...
const CustomControls = (collapsed, setCollapsed) => ({
...
const AppLayout = ({ children }) => {
  ...
  const closeCollapsed = useCallback(() => {
    setCollapsed(false);
  }, []);
      ...
      {isTabletPC ? (
        <Layout className="layout">
          ...
          >
            <Logo>
              ...
              <Row gutter={3}>
                <Space style={{ fontSize: 13 }} direction="vertical">
                  <>
          ...
          <Layout style={{ overflow: 'unset' }}>
            <Content>
              <div>
                <FullPage controls={CustomControls(collapsed, setCollapsed)}>
                  {children}
                </FullPage>
              </div>
            </Content>
            <Footer>
              <p>Juyoung's Portpolio ©2020 Created by Juyoung Jung</p>
            </Footer>
          </Layout>
        </Layout>
      ) : (
        <Layout className="layout">
          ...
          <Header className="layout-background">
            ...
            >
              {React.createElement(
                collapsed ? MinusCircleTwoTone : PlusCircleTwoTone,
                {
                  className: 'trigger',
          ...
          <Content onClick={closeCollapsed}>
            <div>
              <FullPage controls={CustomControls(collapsed, setCollapsed)}>
                {children}
              </FullPage>

const projectData = [
    {
        ...
        desc:
            "React, Next, Redux, Redux Saga, Express를 이용해 Fullstack으로 만든, 트위터의 기능을 가진 SNS 서비스입니다",
    ...
    },
    {
        ...
        desc:
            "`오늘은 어디 밀롱가를 갈까?!`를 콘셉으로 Kakao Map API를 활용해 만든 장소검색 서비스입니다. 서울에 있는 10곳의 밀롱가를 검색할 수 있습니다.",
        term: "2020.10.14 - 2020.10.27",
        skills: ["html5, css3, sass, javascript, git, webpack"],
        func:
        ...
        details: ` 밀롱가는 탱고라는 Social Dance를 추는 장소입니다. 서울에 있는 10곳의 밀롱가를 JSON형식의 데이터로 가져와 Kakao map에서 검색할 수 있는 반응형 사이트를 구현했습니다. 이 프로젝트를 통해 무엇보다 javascript의 실행 context를 파악하는 연습을 할 수 있었습니다. 
         사용자가 본인의 현 위치를 파악할 수 있도록 GeoLocation을 이용해서 얻어온 접속 위치를 검색한 결과 좌표와 함께 연결하는 과정에서 (1) json 데이터를 검색하는 함수, (2) map의 중심좌표를 이동시키고 마커를 표시하는 함수, (3) 마커에 여러 개의 Event를 등록하는 함수, 그리고 (4) 사용자의 현 위치를 파악해 좌표로 표시하는 총 4개의 함수를 서로 연결시켜야했기 때문입니다. index와 map이라는 두 페이지에 각각 장소검색 기능과 드래그 기능을 적용하는 과정에서 각 페이지의 테그 명이 달라 페이지마다 함수를 따로 적용시키면서 코드량이 늘어나는 문제가 있었습니다. 이를 해결하기위해 module system을 도입해 중복되는 함수를 모듈로 쪼개어 webpack으로 bundling하였습니다. 
         `,
    ...
    },
    {
        ...
        desc:
            "웹페이지만 봐도 여행을 떠나고 싶다!를 컨셉으로 팬션 전경을 화면 가득 보여주는 것을 중점적으로 리뉴얼한 반응형 웹페이지입니다.",
        term: "2020.07.09 - 2020.09.28",
        skills: [
            "html5, css3, sass, javascript, jquery, git, eslint, prettier",
        ],
        func:
